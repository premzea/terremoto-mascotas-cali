...
~/.gstack/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**If A (Retry)**: re-run `~/.gstack/bin/gstack-gbrain-detect`
...
```bash
. ~/.gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
_HASH=$(~/.gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
...
```bash
~/.gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
