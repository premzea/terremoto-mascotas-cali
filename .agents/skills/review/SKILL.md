...
~/.gstack/bin/gstack-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.gstack/bin/gstack-question-log '{"skill":"review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.gstack/bin/gstack-learnings-log '{
  "type": "pitfall",
...
QUEUE_JSON=$(bun run ~/.gstack/bin/gstack-next-version \
  --base "$BASE_BRANCH" \
  --bump patch \
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.gstack/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
...
```bash
source <(~/.gstack/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.gstack/bin/gstack-specialist-stats 2>/dev/null || true
```
...
```bash
~/.gstack/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
_TEL=$(~/.gstack/bin/gstack-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(~/.gstack/bin/gstack-config get codex_reviews 2>/dev/null || echo enabled)
source ~/.gstack/bin/gstack-codex-probe 2>/dev/null || true
...
```bash
~/.gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.gstack/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
~/.gstack/bin/gstack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
